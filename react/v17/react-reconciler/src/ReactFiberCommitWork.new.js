...
import {commitUpdateQueue} from './ReactUpdateQueue.new'; // 用于类组件setState第二个参数和ReactDOM.render第三个参数
import {
  getPublicInstance,
...
function commitBeforeMutationEffects_complete() {
  while (nextEffect !== null) { // 当前需要处理的fiber
    const fiber = nextEffect;
    ...
    // 处理完当前需要处理的fiber后将当前需要处理的fiber重新设为原fiber的父结点，处理父结点
...
function commitHookEffectListUnmount(
  ...
  if (lastEffect !== null) {
    ...
    do {
      if ((effect.tag & flags) === flags) { // 调用useEffect删除机制
        // Unmount
...
function commitHookEffectListMount(tag: number, finishedWork: Fiber) {
  const updateQueue: FunctionComponentUpdateQueue | null = (finishedWork.updateQueue: any);
  const lastEffect = updateQueue !== null ? updateQueue.lastEffect : null;
  if (lastEffect !== null) { // 有effect
    const firstEffect = lastEffect.next;
    let effect = firstEffect;
    do {
      // tag表示effect的执行阶段（Passive或者Layout）和是否需要触发（HookPassive | HookHasEffect）/（HookLayout | HookHasEffect）
      // 例如useEffect(create:()=>{},deps:[])的deps没有变化，
      // effect.tag上面没有HookHasEffect的tag,所以effect.tag & tag !== tag，不会执行回调函数
      if ((effect.tag & tag) === tag) { // 寻找对应tag的effect执行effect（HookPassive，HookLayout）
        // Mount
        const create = effect.create;
        effect.destroy = create(); // 执行useEffect的回调函数，如果返回一个操作函数为useEffect的清除机制，React 会在组件卸载的时候执行清除操作
      }
      effect = effect.next;
    } while (effect !== firstEffect); // 遍历环状链表
  }
}
    ...
    switch (finishedWork.tag) {
      ...
      case ClassComponent: {
        ...
        if (updateQueue !== null) {
          ...
          // but instead we rely on them being set during last render.
          // TODO: revisit this when we implement resuming.
           // 调用updateQueue的effects数组中的update储存的setState的第二个回调函数参数，将updateQueue的effects设置为null
      ...
      case HostRoot: {
        ...
        if (updateQueue !== null) {
          ...
          // 调用updateQueue的effects数组中的update储存的ReactDOM.render的第三个回调函数参数，将updateQueue的effects设置为null
          commitUpdateQueue(finishedWork, updateQueue, instance);
        }
        break;
      }
      case HostComponent: {
        ...
        if (current === null && finishedWork.flags & Update) {
          ...
          commitMount(instance, type, props, finishedWork); // 更新结点
        }
...
function commitAttachRef(finishedWork: Fiber) { // 生成DOM节点后为ref赋值
  const ref = finishedWork.ref;
  if (ref !== null) {
    const instance = finishedWork.stateNode;
    let instanceToUse;
    switch (finishedWork.tag) {
      case HostComponent: // DOM结点类型
        instanceToUse = getPublicInstance(instance); // DOM结点
        break;
      default:
    ...
    if (typeof ref === 'function') { // 回调 Refs
      if (
        enableProfilerTimer &&
        enableProfilerCommitHooks &&
        finishedWork.mode & ProfileMode
...
function commitUnmount(
  ...
  switch (current.tag) {
    ...
    case ClassComponent: { // 类组件umount
      safelyDetachRef(current, nearestMountedAncestor); // 解绑，ref(null)或ref.current = null
      const instance = current.stateNode;
      if (typeof instance.componentWillUnmount === 'function') { // 调用componentWillUnmount
...
function commitNestedUnmounts(
  ...
  let node: Fiber = root;
  while (true) { // 将需要删除的结点当作根结点，遍历整棵树，为其与其下的每个结点执行commitUnmount
...
function commitPlacement(finishedWork: Fiber): void { // 对于新增和位置错乱的fiber，append到最后一个位置正常的fiber对应的dom
  if (!supportsMutation) {
    return;
  }
  ...
  if (parentFiber.flags & ContentReset) { // 纯文本直接重设
  ...
  const before = getHostSibling(finishedWork); // 找到没有Placement标记的后面的兄弟结点
  // We only have the top Fiber that was inserted but we need to recurse down its
  // children to find all the terminal nodes.
  if (isContainer) {
...
function insertOrAppendPlacementNodeIntoContainer(
  ...
  if (isHost) {
    ...
    if (before) {
      insertInContainerBefore(parent, stateNode, before); // 插入到before结点前
    } else { // 追加到最后
...
function commitDeletion( // 删除结点，函数组件会执行layout effect的销毁机制后删除结点。类组件会执行componentWillUnmount后删除结点
  finishedRoot: FiberRoot,
  current: Fiber,
  nearestMountedAncestor: Fiber,
): void {
...
function commitWork(current: Fiber | null, finishedWork: Fiber): void { // 更新结点
  ...
  switch (finishedWork.tag) {
    ...
    case ForwardRef:
    case MemoComponent:
    case SimpleMemoComponent: { // 函数组件更新，执行layout effect销毁机制
      // Layout effects are destroyed during the mutation phase so that all
      // destroy functions for all fibers are called before any create functions.
      // This prevents sibling component effects from interfering with each other,
      // e.g. a destroy function in one component should never override a ref set
...
// 处理fiber纯文本结点ContentReset的替换（当遍历到只有一个子文本节点的Fiber时，该Fiber节点的子节点不会执行beginWork和completeWork），原current fiber树ref的解绑（ref(null)/ref.current=null）,结点的创建和更新
...
function commitLayoutMountEffects_complete(
  ...
  while (nextEffect !== null) {
    const fiber = nextEffect;
    if ((fiber.flags & LayoutMask) !== NoFlags) { // const LayoutMask = Update | Callback | Ref | Visibility;
      const current = fiber.alternate;
      setCurrentDebugFiberInDEV(fiber);
      try {
...
function commitPassiveMountEffects_begin(subtreeRoot: Fiber, root: FiberRoot) {
  while (nextEffect !== null) {
    const fiber = nextEffect;
    const firstChild = fiber.child;
    if ((fiber.subtreeFlags & PassiveMask) !== NoFlags && firstChild !== null) { // 有passive effect
      ensureCorrectReturnPointer(firstChild, fiber);
      nextEffect = firstChild;
    } else {
