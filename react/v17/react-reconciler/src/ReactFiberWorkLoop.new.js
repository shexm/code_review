...
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  ...
  let newCallbackNode;
  if (newCallbackPriority === SyncLane) { // 同步任务
    ...
    } else { // performSyncWorkOnRoot任务推进syncQueue数组
    ...
    if (supportsMicrotasks) {
      ...
      if (__DEV__ && ReactCurrentActQueue.current !== null) {
        ...
        scheduleMicrotask(flushSyncCallbacks); // 微任务执行掉syncQueue数组中的同步任务
      }
    } else {
      // Flush the queue in an Immediate task.
      // 不支持微任务，将syncQueue数组中的同步任务以scheduler最高的优先级一次性立即执行
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (didFlushPassiveEffects) {
    ...
    // 注意，这里并没有调用ensureRootIsScheduled，因为root.callbackNode !== originalCallbackNode说明有新任务被调度或者wip树已经构建完成
...
function performSyncWorkOnRoot(root) {
  ...
  flushPassiveEffects(); // 副作用

  let lanes = getNextLanes(root, NoLanes);
  if (!includesSomeLane(lanes, SyncLane)) { // lanes上没有SyncLane优先级的lane，没有同步任务
    // There's no remaining sync work left.
    ensureRootIsScheduled(root, now());
    return null;
  }

  let exitStatus = renderRootSync(root, lanes); // 同步优先级任务不使用时间片
...
function commitRootImpl(root, renderPriorityLevel) {
  do {
    ...
    flushPassiveEffects();
  } while (rootWithPendingPassiveEffects !== null); // 执行passive effect
  ...
  const lanes = root.finishedLanes; // 完成构建使用的lanes
  ...
  if (subtreeHasEffects || rootHasEffect) { // 需要触发effect
    const prevTransition = ReactCurrentBatchConfig.transition;
    ReactCurrentBatchConfig.transition = 0;
    const previousPriority = getCurrentUpdatePriority();
    setCurrentUpdatePriority(DiscreteEventPriority);
    ...
    // root.current变成mutation处理完后的wip树
...
function flushPassiveEffectsImpl() {
  ...
  executionContext |= CommitContext;
  // 每次更新的时候都要运行 effect
  commitPassiveUnmountEffects(root.current); // 执行effect清除机制
  commitPassiveMountEffects(root, root.current); // 执行effect回调函数

  executionContext = prevExecutionContext;

  flushSyncCallbacks(); // 执行掉同步优先级的任务，可能产生新的effects
