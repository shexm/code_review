...
export function pushProvider<T>(
  ...
  if (isPrimaryRenderer) { // 浏览器renderer
...
function propagateContextChange_eager<T>(
  ...
  while (fiber !== null) {
    ...
    // Visit this fiber.
    // context的依赖链表，调用readContext（包含static contextType的类组件或者使用useContext的函数组件都会调用readContext）的组件会生成依赖链表
    ...
    if (list !== null) {
      ...
      while (dependency !== null) {
        ...
        if (dependency.context === context) { // 如组件包含context发生变化的Provider，生成update，调度更新任务
