...
export function bailoutHooks(
  ...
  workInProgress.flags &= ~(PassiveEffect | UpdateEffect); // 删除PassiveEffect，UpdateEffect
...
function mountWorkInProgressHook(): Hook {
  const hook: Hook = {
    ...
    next: null, // 连接其他mountXXX产生的hook（在创建函数组件fiber中同时使用多个useXXX）
  };
...
function updateWorkInProgressHook(): Hook {
  ...
  if (nextWorkInProgressHook !== null) {
    ...
    const newHook: Hook = { // 类似于类组件fiber.updateQueue
      ...
      queue: currentHook.queue, // 类似于fiber.updateQueue.shared
...
function updateReducer<S, I, A>(
  ...
  const hook = updateWorkInProgressHook(); // 根据current树中的hook链表同步更新wip树中的hook链表,根据react hooks的执行递进地获取当前hook在链表中的节点
  const queue = hook.queue;
  invariant(
    queue !== null,
  ...
  let baseQueue = current.baseQueue; // 上次未完成的更新

  // The last pending update that hasn't been processed yet.
  const pendingQueue = queue.pending; // 本次新增加的更新
  if (pendingQueue !== null) { // 本次新增加了更新
    // We have new updates that haven't been processed yet.
    // We'll add them to the base queue.
    if (baseQueue !== null) { // 上次有未完成的更新 由pendingQueue环状链表生成
    ...
    do {
      ...
      if (!isSubsetOfLanes(renderLanes, updateLane)) {
        ...
        if (newBaseQueueLast === null) {
          ...
          newBaseState = newState; // 记录上次的state
        } else {
        ...
        if (update.eagerReducer === reducer) {
          ...
          const action = update.action; // state => state / state
...
function pushEffect(tag, create, destroy, deps) { // 更新fiber.updateQueue
  // HookHasEffect | hookFlags,create,undefined,nextDeps,
  const effect: Effect = {
    tag,
    create, // useEffect回调
    destroy,
  ...
  if (componentUpdateQueue === null) {
    ...
    componentUpdateQueue.lastEffect = effect.next = effect; // 自己指向自己成为环状链表
  } else { // 环状链表
    const lastEffect = componentUpdateQueue.lastEffect; // 最新的effect
    if (lastEffect === null) {
...
function mountEffectImpl(fiberFlags, hookFlags, create, deps): void {
  ...
  hook.memoizedState = pushEffect( // fiber.updateQueue添加effect，hook.memoizedState为添加的effect
    HookHasEffect | hookFlags, // 需要触发effect，Layout/Passive阶段触发effect
    create,
    undefined,
    nextDeps,
  );
...
function updateEffectImpl(fiberFlags, hookFlags, create, deps): void {
  // PassiveEffect, HookPassive, create, deps
  const hook = updateWorkInProgressHook();
  const nextDeps = deps === undefined ? null : deps;
  ...
  if (currentHook !== null) {
    ...
    // commitHookEffectListMount添加,commitHookEffectListUnmount执行
    destroy = prevEffect.destroy;
    if (nextDeps !== null) {
      const prevDeps = prevEffect.deps;
      if (areHookInputsEqual(nextDeps, prevDeps)) {
        hook.memoizedState = pushEffect(hookFlags, create, destroy, nextDeps); // 没有HookHasEffect代表不需要触发effect
        return;
      }
    }
  }
  ...
  hook.memoizedState = pushEffect(
    HookHasEffect | hookFlags, // HookHasEffect代表需要触发effect
    create,
    destroy,
    nextDeps,
  );
...
function mountEffect(
  ...
  deps: Array<mixed> | void | null,
    ...
    return mountEffectImpl(
      PassiveEffect | PassiveStaticEffect, // effect类型 副作用effect
      HookPassive, // Passive阶段触发
      create,
      deps,
    );
}
...
function mountLayoutEffect(
  ...
  let fiberFlags: Flags = UpdateEffect; // 更新类型effect
  if (enableSuspenseLayoutEffectSemantics) {
    fiberFlags |= LayoutStaticEffect;
  }
  return mountEffectImpl(fiberFlags, HookLayout, create, deps); // Layout阶段触发
}
...
function dispatchAction<S, A>(
  fiber: Fiber,
  queue: UpdateQueue<S, A>, // useState的queue是hook.queue
  action: A,
) {

  const eventTime = requestEventTime();
  const lane = requestUpdateLane(fiber); // 事件优先级
  ...
  // 现在current fiber对应的wip fiber commit阶段wip树会变成current树，
  // 所以更新的时候原来bind的wip fiber参数成为现在的current fiber
  ...
  if (
    ...
    if (pending === null) { 
      // mountState时为null 和类组件setState产生的update一样使用环状链表储存update，最开始update只有一个时自己链接自己生成环状链表
    ...
    const root = scheduleUpdateOnFiber(fiber, lane, eventTime); // 合并lane到root，调度任务
