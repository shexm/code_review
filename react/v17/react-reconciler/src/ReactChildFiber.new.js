...
function ChildReconciler(shouldTrackSideEffects) { //  update为副作用
  ...
  function useFiber(fiber: Fiber, pendingProps: mixed): Fiber {
    ...
    // 新的fiber排列位置还没有确定,在ReactElement和oldFiber对比中确定index和sibling
    clone.index = 0;
    clone.sibling = null;
    return clone;
  }
  ...
  function reconcileChildrenArray(
    ...
    for (; oldFiber !== null && newIdx < newChildren.length; newIdx++) {
      ...
      // 如果ReactElement和oldFiber的key不同返回null，结束第一个循环，会执行第三个循环
...
export function cloneChildFibers(
  ...
  let currentChild = workInProgress.child; // current
