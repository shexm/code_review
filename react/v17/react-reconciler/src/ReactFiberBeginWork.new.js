...
function updateMemoComponent(
  ...
  Component: any,
  nextProps: any,
  updateLanes: Lanes, // workInProgress.lanes
  renderLanes: Lanes,
): null | Fiber {
  if (current === null) { // 创建
  ...
  if (!includesSomeLane(updateLanes, renderLanes)) { // 组件没有产生updateLanes或者updateLanes优先级不足
    ...
    if (compare(prevProps, nextProps) && current.ref === workInProgress.ref) { // 浅比较props和ref，没有变化跳过更新
      return bailoutOnAlreadyFinishedWork(current, workInProgress, renderLanes);
    }
  }
  // React DevTools reads this flag.
...
function markRef(current: Fiber | null, workInProgress: Fiber) {
  const ref = workInProgress.ref;
  if (
    (current === null && ref !== null) || // 挂载时有ref
    (current !== null && current.ref !== ref) // 更新时ref发生了变化
  ) {
...
function updateClassComponent(
  ...
  let shouldUpdate;
  if (instance === null) { // wip没有组件实例
    if (current !== null) { // 有currentFiber
...
function finishClassComponent(
  ...
  if (!shouldUpdate && !didCaptureError) { // 不需要更新没有异常到捕获
...
function updateContextProvider(
  ...
  const newValue = newProps.value; // provider上的value
...
function bailoutOnAlreadyFinishedWork(
  ...
  if (!includesSomeLane(renderLanes, workInProgress.childLanes)) { // 子树也不需要更新，beginWork遍历到了最底层，应该执行completeWork
...
function beginWork(
  ...
  if (current !== null) { // 更新
    ...
    if (
      ...
      switch (workInProgress.tag) { // 没有函数组件
        case HostRoot:
          pushHostRootContext(workInProgress);
          if (enableCache) {
            const root: FiberRoot = workInProgress.stateNode;
